...
/**
 * Module:    Đặt lịch thực hiện công việc, hỗ trợ
 * Author:    Tinasoft.vn
 * @version:  v1.0.0
...
function convertCronTab(expression, number) {
	try {
		...
		for (let i = 0; i < number; i++){
			console.log(`Lần tiếp (${i + 1}) :`, interval.next().toString());
		}
	} catch (err) {
		throw err
	}
 ...
 * @returns {Error[]|*[id công việc]}
 */

function setTimeToDoJob(jobId, datetime, job) {
	try {
		...
		schedule.scheduleJob(jobId, setTime, job);
		return [null, jobId]
	} catch (e) {
		debug(e);
		return [e]
