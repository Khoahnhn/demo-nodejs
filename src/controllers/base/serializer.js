...
async function convertOutput(object) {
    ...
    if(object.image) object.image = getPhoto(object.image);
    if(object.owner_id) {
...
function getPhoto(img_path){
    let splited_path = img_path.split('\\'); // mot mang 2 phan tu
    let photo_url = splited_path.slice(0, splited_path.length).join('/');
    return `https://${network.domain}/media/static/${photo_url}`;
}
...
async function getStageSeed(seedId, time){
    try{
        ...
        else if(days>=stage_2_days) stage = stage_3_info;
        return {stage, seed: seed.seed}
    }catch(err){
        throw err
    }
